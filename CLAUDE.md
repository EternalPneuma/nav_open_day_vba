...
| `scripts/tools/verify_yield_formula.py` | 验证 `产品净值汇总_yyyymmdd.xlsx` 中 30 日年化收益率公式（对比单利/复利） |
...
- **收益率公式（单利）**：全部年化收益率使用单利公式 `(V_t / V_base - 1) × (365 / 实际间隔)`，非复利。适用：7日/28日/30日年化、当前/上一周期年化、成立以来年化。`chart/02_output_data.bas` 中 30 日年化匹配基准日期时向前最多 4 次回溯（-30/-31/-32/-33 天）
