Attribute VB_Name = "导出拼接图表"
Option Explicit

'==============================================================
' 模块: 导出拼接图表
' 功能: 导出每个sheet的4个chart, 同色拼接, 单图存raw子文件夹
'==============================================================
...
Private Function MergeTwoImages(ByVal wb As Workbook, _
    ...
    Dim totalW As Single: totalW = leftW + rightW
    ...
    Set shpRight = ch.Shapes.AddPicture( _
        fileName:=rightPath, LinkToFile:=msoFalse, SaveWithDocument:=msoTrue, _
        Left:=leftW, Top:=0, Width:=rightW, Height:=rightH)
