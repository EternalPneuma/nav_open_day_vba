...
'==============================================================
' 模块: 导出拼接图表
' 功能: 导出每个sheet的4个chart, 红色左右拼接, 蓝色上下拼接, 单图存raw子文件夹
'==============================================================
...
Private Function MergeTwoImages(ByVal wb As Workbook, _
    ...
    Dim gap As Single: gap = 15   '设置左右间距为15磅(可按需调整)
...
Private Function MergeTwoImagesVertical(ByVal wb As Workbook, _
    ...
    Dim tmpSheet As Worksheet
    Set tmpSheet = wb.Sheets.Add(After:=wb.Sheets(wb.Sheets.Count))
    tmpSheet.Name = "_tmpV_" & Format(Timer * 1000, "0")
  
    Dim probeTop As Shape, probeBottom As Shape
    Set probeTop = tmpSheet.Shapes.AddPicture( _
        fileName:=topPath, LinkToFile:=msoFalse, SaveWithDocument:=msoTrue, _
        Left:=0, Top:=0, Width:=-1, Height:=-1)
    Set probeBottom = tmpSheet.Shapes.AddPicture( _
        fileName:=bottomPath, LinkToFile:=msoFalse, SaveWithDocument:=msoTrue, _
        Left:=0, Top:=0, Width:=-1, Height:=-1)
  
    Dim topW As Single: topW = probeTop.Width
    Dim topH As Single: topH = probeTop.Height
    Dim bottomW As Single: bottomW = probeBottom.Width
    Dim bottomH As Single: bottomH = probeBottom.Height
  
    Dim totalW As Single
    totalW = IIf(topW > bottomW, topW, bottomW)
    Dim gap As Single: gap = 15   '设置上下间距为15磅(可按需调整)
    Dim totalH As Single: totalH = topH + bottomH + gap
  
    ' 删除探测用的图片
    probeTop.Delete
    probeBottom.Delete
  
    ' 2. 创建chart容器, 尺寸=最大宽×总高
    Dim co As ChartObject
    Set co = tmpSheet.ChartObjects.Add( _
        Left:=0, Top:=0, Width:=totalW, Height:=totalH)
    ...
    ' 3. 直接在chart内部添加两张图作为shape (上下布局)
    Dim shpTop As Shape, shpBottom As Shape
    Set shpTop = ch.Shapes.AddPicture( _
        fileName:=topPath, LinkToFile:=msoFalse, SaveWithDocument:=msoTrue, _
        Left:=0, Top:=0, Width:=topW, Height:=topH)
  
    Set shpBottom = ch.Shapes.AddPicture( _
        fileName:=bottomPath, LinkToFile:=msoFalse, SaveWithDocument:=msoTrue, _
        Left:=0, Top:=topH + gap, Width:=bottomW, Height:=bottomH)
