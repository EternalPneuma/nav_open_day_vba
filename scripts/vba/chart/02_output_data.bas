...
NextProduct:
    ...
    WriteDataSummarySheet wbOut, dimDict, groupDict, g_fillCounts
    ...
    If Len(emptyProducts) > 0 Then
        ...
        msg = msg & vbCrLf & vbCrLf & "产品信息中存在但绘图净值数据无数据(已跳过):" & vbCrLf & emptyProducts
    End If
    
    If Len(missingProducts) > 0 Then
        If Len(missingProducts) > 2 Then missingProducts = Left(missingProducts, Len(missingProducts) - 2)
        msg = msg & vbCrLf & vbCrLf & "绘图净值数据中存在但产品信息未配置(已跳过):" & vbCrLf & missingProducts
    End If
...
'==============================================================
Private Sub WriteDataSummarySheet(ByVal wbOut As Workbook, ByVal dimDict As Object, ByVal groupDict As Object, ByVal fillDict As Object)
        ...
        For Each pkKey In subDict.keys
            ...
            If IsDate(rowArr(1)) And IsNumeric(rowArr(4)) Then
                If CDbl(rowArr(4)) > 0 Then
                    idx = idx + 1
                    dArr(idx) = CLng(CDbl(CDate(rowArr(1))))
